...
Berlin serves as a useful microcosm for voting trends at the national level in Germany for several reasons:
When plotting the voting results across districts in Berlin, the East/West divide (the same division on the national level) is one of the most prominent features of the data \--and yet Berlin offers publicly available geo-data that can be used to `map' voting trends across various districts.
